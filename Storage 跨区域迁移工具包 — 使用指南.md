...
Azure China East (CE1)/China North 1 (CN1)数据中心将于 **2026-07-01 关闭**，客户需将 Blob Storage 和 Azure Files 数据及时迁出。
...
| `STORAGE_TYPE`       | `blob` / `files-smb` / `files-nfs` | `blob`             |
...
> [!tip] Azure Files 权限自动保留
> 脚本根据 `STORAGE_TYPE` 自动添加对应的权限保留参数，无需手动配置：
> - `files-smb`：添加 `--preserve-smb-permissions` + `--preserve-smb-info` → 保留 NTFS ACL 和 SMB 属性
> - `files-nfs`：添加 `--preserve-permissions` + `--preserve-info` + `--from-to=FileNFSFileNFS` → 保留 POSIX uid/gid/mode
